...
import {
  Users, Search, Filter, Phone, Calendar,
  ChevronDown, ChevronRight, Star, StarOff,
...
const EmployerApplications = () => {
  ...
  const fetchApplications = async () => {
    ...
    // 1. Get all jobs for this employer (including skills for match calculation)
    ...
    jobs.forEach(j => { jobSkillsMap[j.id] = Array.isArray(j.skills) ? j.skills : []; });

    // 2. Get all applications with candidate profile
    const { data: appData, error: appError } = await supabase
      ...
      .select(`
        ...
        profiles!applications_jobseeker_id_fkey (
          id, full_name, phone, skills, resume_url, headline
        )
      `)
      .in("job_id", jobIds)
    ...
    const formatted = (appData || []).map((app: any) => {
      const candidateSkills = Array.isArray(app.profiles?.skills) ? app.profiles.skills : [];
      const jobSkills       = jobSkillsMap[app.job_id] || [];
      ...
      return {
        ...
        candidate: {
          ...
          avatar:   (app.profiles?.full_name || "?")
            .split(" ").map((n: string) => n[0]).join("").toUpperCase().slice(0, 2),
        },
        // ✅ Real match score — same algorithm as jobseeker side
        matchScore: calcMatch(jobSkills, candidateSkills),
        isStarred:  false,
      };
    });
    ...
    if (error) {
      ...
      setApplications(prev =>
        prev.map(a => a.id === applicationId
          ? { ...a, status: newStatus, statusLabel: statusLabels[newStatus] || newStatus }
          : a
        )
      );
  ...
  const toggleStar = (id: string) => {
    setApplications(prev => prev.map(a => a.id === id ? { ...a, isStarred: !a.isStarred } : a));
  };
  ...
  const stageCounts = (stage: string) =>
    stage === "all" ? applications.length : applications.filter(a => a.status === stage).length;

  const pipelineStages = ["all","applied","reviewed","shortlisted","interview","accepted","rejected"];
    ...
    <EmployerLayout>
      <div className="min-h-screen bg-background">
        ...
        <main className="container py-8">
              ...
              <TabsList className="w-full justify-start overflow-x-auto flex-nowrap bg-card border border-border h-auto flex-wrap gap-1 p-1">
                {pipelineStages.map(stage => (
                  ...
                  >
                    ...
                    <Badge variant="secondary" className="text-xs">{stageCounts(stage)}</Badge>
                  </TabsTrigger>
                ))}
              </TabsList>
            </Tabs>
          ...
          {/* Filters */}
          <motion.div className="flex flex-col md:flex-row gap-4 mb-6" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
          ...
          {loading ? (
            ...
            <div className="text-center py-16">
              ...
              <p className="text-muted-foreground">
                {applications.length === 0 ? "No one has applied to your jobs yet." : "Try adjusting your search or filters."}
              </p>
            </div>
          ) : (
                  ...
                  <Card className="border-0 shadow-lg overflow-hidden">
                    <CardContent className="p-0">
                      ...
                      >
                        ...
                        {/* ✅ Real match score with colour coding */}
                        <div className="text-center px-4 shrink-0">
                          <p className={`text-2xl font-bold ${getMatchColor(application.matchScore)}`}>
                        ...
                        <div className="text-muted-foreground shrink-0">
                          {expandedCard === application.id ? <ChevronDown className="w-5 h-5" /> : <ChevronRight className="w-5 h-5" />}
                        </div>
                      </div>
                      ...
                      {expandedCard === application.id && (
                        ...
                        >
                          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            {/* Candidate Details */}
                            <div>
                              <h4 className="text-sm font-semibold text-foreground mb-3">Candidate Details</h4>
                              <div className="space-y-2">
                                {application.candidate.phone && (
                                  <a href={`tel:${application.candidate.phone}`} className="flex items-center gap-2 text-sm text-muted-foreground hover:text-primary transition-colors">
                                    <Phone className="w-4 h-4" /> {application.candidate.phone}
                                  </a>
                                )}
                                {application.candidate.skills.length > 0 && (
                                  <div>
                                    ...
                                    <div className="flex flex-wrap gap-1">
                                      {application.candidate.skills.slice(0, 6).map((s: string) => (
                                ...
                                {application.coverLetter && (
                                  <div>
                                    ...
                                    <p className="text-sm text-foreground line-clamp-3">{application.coverLetter}</p>
                                  </div>
                                )}
                              </div>
                            </div>
                            ...
                            <div>
                              ...
                              <div className="flex flex-wrap gap-2">
                                {application.candidate.id && (
                                  <Link to={`/employer/candidate/${application.candidate.id}`}>
                            ...
                            <div>
                              ...
                              <p className="text-xs text-muted-foreground mt-3">
                                Current: <span className="font-medium text-foreground">{application.statusLabel}</span>
                              </p>
                            </div>
                          </div>
                        </motion.div>
